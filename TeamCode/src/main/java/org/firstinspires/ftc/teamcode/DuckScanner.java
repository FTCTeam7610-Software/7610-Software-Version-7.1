...
public class DuckScanner extends OpenCvPipeline {
    ...
    Rect LEFT_ROI = new Rect(new Point(0, 0), new Point(width / 3, 2*height/3));
    Rect MIDDLE_ROI = new Rect(new Point(width/3, 0), new Point(2*width / 3, 2*height/3));
    Rect RIGHT_ROI = new Rect(new Point(2*width/3, 0), new Point(width, 2*height/3));
